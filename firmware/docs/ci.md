...
uv run --with pytest pytest firmware/tests     # 146 tests, ~20-30 s
```
...
- The individual commands in every job were dry-run on
  `desktop.buddy.mithis.com` (firmware build, the 146 host tests, an `idf.py`
  ...
  runner — passing green (build, 146 host tests, and all eight Renode suites).
