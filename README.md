...
| [`hardware/esp32c3-cc1101-adapter`](#adapter-esp32-c3-supermini-to-cc1101-board) | carrier: SuperMini + E07-M1101D socket, single-sided | 30.0 x 38.0 |
...
The SuperMini's pins go into 1.0 mm through-holes (1.6 mm pads) so it can be
soldered flat or fitted with headers; its USB-C connector points off a board
edge and no copper runs under its ceramic antenna. The silkscreen carries the
SuperMini's own pin names on the outer side of each header row and the radio
signal names on the inner side.
...
30 x 38 mm, single-sided: every track is on the back copper and the front
carries only the through-hole pad rings, so it can be etched at home or
ordered as a one-layer board from the back-layer artwork alone. The
SuperMini lies on its side with USB-C off the left edge, which places its
GPIO row directly above the socket and its power row behind it; that is what
lets all eight nets route on one layer without jumpers (MISO and GDO2 pass
over the top of the power row and down the left edge). A 2x4 socket (1.0 mm
holes, same numbering as the E07-M1101D header: pin 1 square at the right
of the outer row) takes the CC1101 board, which stands upright with its SMA
jack away from the carrier. The layout plot shows back-copper tracks in
blue.
