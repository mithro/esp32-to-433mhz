...
| --- | --- | --- | --- |
| MOSI | GPIO5 | 6 MOSI | MOSI |
| SCK | GPIO6 | 5 SCK | SCK |
| Chip select | GPIO7 | 4 CSN | NSS |
| MISO | GPIO4 | 7 MISO | MISO |
| IRQ / packet | GPIO10 | 3 GDO0 | RST (input to the radio; drive it as an output) |
| Second IRQ | GPIO3 | 8 GDO2 | DIO0 (the Ra-02's packet IRQ) |
| 3.3 V | 3V3 | 2 VCC | 3V3 |
| GND | GND | 1 GND | GND |
| Radio-type strap | GPIO1 | JP1 open | JP1 bridged to GND |

GPIO2, GPIO8 and GPIO9 are ESP32-C3 strapping pins, so the radio's interrupt
outputs (which idle low at power-up) avoid them. GPIO1 is the radio-type
strap (below); the other seven unused pins (5V, GPIO0, GPIO2, GPIO8, GPIO9,
GPIO20, GPIO21) are brought out to a 1x7 header on the top edge. The
...
`hardware/esp32c3-radio-adapter`, 27 x 36.5 mm. Every radio track is on the
back copper; the front carries the pad copper, a GND pour (thermal-relieved
to the SuperMini's G pin and the socket's GND pin) and the tracks of the
expansion header and strap, so the back-layer artwork alone still works as
a single-sided board for the header-pin option (without the expansion
header). A keepout on both layers keeps the pour and all tracks away from the
...
The layout plot shows back-copper tracks in blue and front tracks in red.

### Expansion header and radio-type strap

J4 is a 1x7 2.54 mm header on the top edge carrying every SuperMini pin the
radio does not use, in the order 5V, GPIO8, GPIO9, GPIO2, GPIO20, GPIO21,
GPIO0 (pin 1, square, next to the top-left hole; the silk names them the
way the SuperMini does). Its pins sit at the gaps between the SuperMini's
pins, so each net reaches it on the front copper without crossing another:
the power-row nets jog above their row, the GPIO-row nets rise between the
rows and pass between the power-row pads.

JP1, a two-pad solder jumper on the front right of the SuperMini, straps
GPIO1 (net `RADIO_ID`) so firmware can tell which radio it is driving. The
SX1278 module adapter ties the same pin to 3V3, giving three distinct
states:

| GPIO1 reads | Board |
| --- | --- |
| floating | this adapter with the E07-M1101D (CC1101): JP1 open |
| low | this adapter with the Ra-02 breakout: JP1 bridged to GND |
| high | the SX1278 module adapter (GPIO1 hard-wired to 3V3) |

To read it, enable GPIO1's internal pull-down and sample (high means the
SX1278 module adapter), then enable the pull-up and sample again (low means
the Ra-02; high means the pin is floating, i.e. the CC1101). GPIO1 is not an
...
the same GPIO assignment as the pin-header adapter plus GPIO8 for the
...
| MOSI | GPIO5 | 7 MOSI |
| SCK | GPIO6 | 8 SCK |
| Chip select | GPIO7 | 9 NSS |
| MISO | GPIO4 | 6 MISO |
| IRQ / packet | GPIO10 | 10 DIO0 |
| Second IRQ | GPIO3 | 2 DIO1 |
| Reset | GPIO8 | 11 RESET |
| 3.3 V | 3V3 | 5 VCC |
| GND | GND | 1, 12, 15 GND |
| Radio-type strap | GPIO1 | tied to 3V3 (reads high) |

This adapter is two-layer: signals run on the top copper in horizontal
lanes below the SuperMini, power and reset on the bottom copper, with vias
to the SMD module pads. GPIO1 is hard-wired to 3V3 (silk "ID=3V3") so the
