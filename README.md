...
| 3.3 V | 3V3 | 2 VCC | 3V3 |
| GND | GND | 1 GND | GND |
| Radio-type strap | GPIO1 | JP1 open, R1 not fitted | jumper on JP1 or 0R in R1 (to GND) |
...
`hardware/esp32c3-radio-adapter`, 29 x 36.5 mm. Every radio track is on the
...
holes, carrying every SuperMini pin the radio does not use in the order 5V,
GPIO8, GPIO9, GPIO2, GPIO20, GPIO0, GPIO21 (pin 1, square, at the left; the
silk names them the way the SuperMini does). Each net reaches it on the
front copper without crossing another: the GPIO-row nets rise between the
SuperMini rows and pass between the power-row pads, the power-row nets jog
above their row, and GPIO21 goes round the right of the SuperMini.

JP1 and R1, on the front beside the socket, strap GPIO1 (net `RADIO_ID`) to
GND so firmware can tell which radio it is driving: JP1 is a 1x2 2.54 mm
header for an ordinary jumper, R1 an 0805 pad pair in parallel with it for
a 0R resistor as the permanent alternative. The SX1278 module adapter ties
the same pin to 3V3, giving three distinct states:
