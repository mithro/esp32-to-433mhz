...
| [`hardware/esp32c3-sx1278-adapter`](#adapter-esp32-c3-supermini-to-sx1278-module) | carrier: SuperMini + SX1278 module, wire or SMA antenna | 24.0 x 58.0 |
| [`hardware/esp32c3-cc1101-adapter`](#adapter-esp32-c3-supermini-to-cc1101-board) | carrier: SuperMini + E07-M1101D socket, tracks on one layer | 27.0 x 36.5 |
| [`hardware/sx1278-ra02-breakout`](#sx1278-ra-02-breakout) | replica: SX1278 LoRa 433MHz v4.0 breakout (Ra-02 + 2x4 header) | 17.5 x 22.5 |
| [`hardware/esp32c3-ra02-adapter`](#adapter-esp32-c3-supermini-to-sx1278-ra-02-breakout) | carrier: SuperMini + Ra-02 breakout socket, tracks on one layer | 27.0 x 36.5 |
...
The blue "SX1278 LoRa 433MHz v4.0" breakout: an Ai-Thinker Ra-02 LoRa
module (IPEX antenna) on a 17.5 x 22.5 mm carrier with a 2x4 2.54 mm male
header on its back. Drawn as seen from the Ra-02 side with the header edge
at the top, the way it sits on the adapters below.
...
fixed the module's orientation). Both sides carry a GND pour. The
breakout's two decoupling capacitors are not modelled, and the module lands
are drawn 1.4 x 1.2 mm.

## Adapters

Three small carrier boards wire an ESP32-C3 SuperMini to a radio. They share
one GPIO assignment (the Ra-02 breakout adapter differs only in DIO0) so one
firmware SPI setup serves any of them:

| Signal | ESP32-C3 GPIO | SX1278 module pin | E07-M1101D pin | Ra-02 breakout |
| --- | --- | --- | --- | --- |
| MOSI | GPIO5 | 7 MOSI | 6 MOSI | MOSI |
| SCK | GPIO6 | 8 SCK | 5 SCK | SCK |
| Chip select | GPIO7 | 9 NSS | 4 CSN | NSS |
| MISO | GPIO4 | 6 MISO | 7 MISO | MISO |
| IRQ / packet | GPIO10 (Ra-02 adapter: GPIO1) | 10 DIO0 | 3 GDO0 | DIO0 |
| Second IRQ | GPIO3 | 2 DIO1 | 8 GDO2 | - |
| Reset | GPIO8 | 11 RESET | - | RST |
...
24 x 58 mm. The SX1278 module is soldered onto SMD land pads that reproduce
...
the bottom edge. The antenna trace runs to a 1.0 mm hole for a spring
...
27 x 36.5 mm. Every track is on the back copper; the front carries the pad
copper and a GND pour (thermal-relieved to the SuperMini's G pin and the
socket's GND pin), so the back-layer artwork alone still works as a
single-sided board for the header-pin option. A keepout on both layers keeps
the pour and all tracks away from the SuperMini's ceramic antenna (the 3.8 mm
of the module furthest from the USB-C). Silkscreen is printed on both sides
(the back copy mirrored), and there is an M2 mounting hole 2.4 mm in from
each corner.
...
the SuperMini (outside its body) and under the socket to its left column. A 2x4 socket (1.0 mm
...
### Adapter: ESP32-C3 SuperMini to SX1278 Ra-02 breakout

| 3D render, top | 3D render, bottom | Layout (copper, silk, fab, outline) |
| --- | --- | --- |
| ![3D render of the top side](docs/images/esp32c3-ra02-adapter-3d-top.png) | ![3D render of the bottom side](docs/images/esp32c3-ra02-adapter-3d-bottom.png) | ![2D layout plot](docs/images/esp32c3-ra02-adapter-layout.png) |

27 x 36.5 mm, the same carrier as the CC1101 adapter (keyhole SuperMini
pads, corner M2 holes, silk both sides, GND pour on the front with the
antenna keepout, all tracks on the back) with a 2x4 socket for the blue
"SX1278 LoRa 433MHz v4.0" breakout that carries an Ai-Thinker Ra-02 (IPEX
antenna) and a 2x4 male header on its back. The breakout is described (and
replicated) [above](#sx1278-ra-02-breakout); the socket's outer row is MISO,
SCK, RST, GND and the inner row DIO0, MOSI, NSS, 3V3 (pin 1 square at the
left of the outer row, even pins below odd ones). The breakout plugs in
component side up and extends past the bottom edge with its IPEX end away
from the carrier.

DIO0 is on GPIO1 here instead of GPIO10: with this socket pinout the GND and
3V3 drops must pass left of every GPIO-row stub, which GPIO10 sits in the way
of, whereas from the power row DIO0 can travel over the top with MISO. GPIO1
is neither a strapping nor a UART pin. Everything else keeps the shared map.
