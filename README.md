...
outputs (which idle low at power-up) avoid them. GPIO8, GPIO20/21 (UART) and
GPIO0/1/9 stay free. The SuperMini's pins go into 1.0 mm through-holes so it
...
to the SMD module pads. Its manufacturing packages are built alongside the
pin-header adapter's.
