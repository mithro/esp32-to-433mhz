...
| [`hardware/esp32c3-cc1101-adapter`](#adapter-esp32-c3-supermini-to-cc1101-board) | carrier: SuperMini + E07-M1101D socket | 24.0 x 38.0 |
...
soldered flat or fitted with headers; its USB-C connector points off the top
edge of the carrier and no copper runs under its ceramic antenna.

Routing is generated by `scripts/generate_adapters.py`: signals run on the
top copper in horizontal lanes below the SuperMini, power and the awkward
signals run on the bottom copper, and vias join bottom-layer nets to the
SMD module pads. Both boards are 24 mm wide.
...
24 x 38 mm. A 2x4 socket (1.0 mm holes, same numbering as the E07-M1101D
header: pin 1 square at the right of the outer row) takes the CC1101 board,
which stands upright with its SMA jack away from the carrier.
