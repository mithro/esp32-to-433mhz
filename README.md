...
antenna) and a 2x4 male header on its back. Read on the breakout's back with
the header at the bottom, its silk grid is MISO/DIO0, SCK/MOSI, RST/NSS,
GND/3V3 per column, the first name in the row nearest the edge; turned over
onto the carrier that order is unchanged, so the socket's outer row is MISO,
SCK, RST, GND and the inner row DIO0, MOSI, NSS, 3V3 (pin 1 square at the
left of the outer row, even pins below odd ones). The breakout (about
17.5 x 22.5 mm, measured from photos) plugs in component side up and extends
past the bottom edge with its IPEX end away from the carrier.
