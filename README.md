...
| [`hardware/esp32c3-radio-adapter`](#adapter-esp32-c3-supermini-to-cc1101-board-or-ra-02-breakout) | carrier: SuperMini + one socket for the E07-M1101D or the Ra-02 breakout, tracks on one layer | 27.0 x 36.5 |
...
Two small carrier boards wire an ESP32-C3 SuperMini to a radio: one carries
the castellated SX1278 module, the other has a 2x4 socket that takes either
the CC1101 board or the Ra-02 breakout. They share one GPIO assignment so
one firmware SPI setup serves any radio; only the socket's two
board-dependent pins change meaning:
...
carries a ground pour on the front (see below).
...
| Socket position | E07-M1101D pin | Ra-02 breakout pin | ESP32-C3 GPIO on the radio adapter |
| --- | --- | --- | --- |
| Outer col 4 | 1 GND | 7 GND | GND |
| Inner col 4 | 2 VCC | 8 3V3 | 3V3 |
| Inner col 2 | 6 MOSI | 4 MOSI | GPIO5 |
| Outer col 2 | 5 SCK | 3 SCK | GPIO6 |
| Inner col 3 | 4 CSN | 6 NSS | GPIO7 |
| Outer col 1 | 7 MISO | 1 MISO | GPIO4 |
| Outer col 3 | 3 GDO0 (output) | 5 RST (input) | GPIO10 |
| Inner col 1 | 8 GDO2 (output) | 2 DIO0 (output) | GPIO3 |

Because the two differing positions are wired to plain GPIOs (neither is a
strapping pin), one socket serves both boards: firmware treats GPIO10 as an
interrupt input for the CC1101 and as the reset output for the Ra-02, and
reads the Ra-02's DIO0 on GPIO3.
...
### Adapter: ESP32-C3 SuperMini to CC1101 board or Ra-02 breakout
