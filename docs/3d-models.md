...
[Printing a case](../README.md#printing-a-case) has the closed case with
the E07-M1101D, the open one with the Ra-02 and the halves on their own.
The design views, with either radio:
...
| Feature | Value |
| --- | --- |
| Cavity | x -2.3 to 29.5, y -0.5 to 61.5, z -3.1 to 9.6 (the USB-C face, the trimmed pin stubs and back-side 0805s under the board, and the J4/J5 pins, with 0.3 to 1.1 mm to spare). Every through-hole pin is assumed trimmed to a 1 mm stub under the board, i.e. to z -2.6 |
| Walls, floor, ceiling | 2.2 / 2.0 / 2.0 mm; the antenna wall 2.5 mm, y 61.5 to 64.0 |
| Outside | 36.2&nbsp;x&nbsp;66.7&nbsp;x&nbsp;16.7 mm, vertical corners R2 |
| Parting line | z = 3.3, on the antenna axis, so the antenna hole and the USB-C window are half in each half: the connectors lie in the bottom half's cut-outs and the top half closes over them. Bottom half 8.4 mm tall (13.4 with its tabs), top half 8.3 (11.4 with its bosses) |
| Standoffs and pegs | 4.0 mm standoffs under the four M2 holes, floor to the board's underside (1.5 mm), each with a 2.15 mm peg, chamfered, standing 0.4 mm proud of the board (flush at the bottom-left hole). FDM pegs print a touch oversize, which is the press fit into the 2.2 mm holes. 4 mm rather than 5 so the bottom-left one clears JP1's pin 2. |
